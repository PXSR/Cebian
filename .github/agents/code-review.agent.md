...
4. **Architecture** — evaluate cohesion, coupling, file size, and placement:
   ...
   - **Coupling & dependency direction** — the Cebian source layers follow a strict one-way dependency chain:
     ...
     - **Pre-existing known violations** — `lib/ui/dialog.ts` (← `components/dialogs` type) and `lib/tools/ask-user-registry.tsx` (← `components/chat/Message` value). These are historical and **not the current change's problem**. Only flag them if the current change touches or extends them.
   ...
   - **`lib/` internal organization** — `lib/` is organized **by concept, not by execution context** (full rules in the **"`lib/` internal organization"** section of [copilot-instructions.md](../copilot-instructions.md)). When the change adds or moves a file under `lib/`, verify:
