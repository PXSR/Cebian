...
export function MessageMetaRow({
  ...
  // copy button is rendered. With `justify-between`, an absent copy button
  // would let the lone span flush left — wrong for an "end-of-turn" footer.
  return (
    <div className="mt-2 flex items-center gap-2 text-[0.7rem] text-muted-foreground/70">
      {text && <CopyButton text={text} />}
      <span className="ml-auto font-mono">{parts.join(' · ')}</span>
    </div>
  );
}
